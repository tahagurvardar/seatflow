  ...
  return (
    <section className="bg-slate-50 py-14 sm:py-20">
      <Container>
        ...
        <div className="mt-6 flex flex-wrap gap-3 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><Link href={ROUTES.organizerEvents(selectedMembership.organization.slug)} className={buttonStyles()}>Manage events</Link><Link href={ROUTES.organizerApprovedVenues(selectedMembership.organization.slug)} className={buttonStyles({ variant: "outline" })}>Review approved venues</Link><p className="basis-full text-sm text-slate-600">Booking, ticket sales, and revenue metrics are intentionally unavailable in Phase 4A. Session pages show aggregate inventory and temporary holds only.</p></div>
      </Container>
    </section>
  );
}
