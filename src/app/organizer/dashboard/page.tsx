  ...
  return (
    <section className="bg-slate-50 py-14 sm:py-20">
      <Container>
        ...
        <div className="mt-6 flex flex-wrap gap-3 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><Link href={ROUTES.organizerEvents(selectedMembership.organization.slug)} className={buttonStyles()}>Manage events</Link><Link href={ROUTES.organizerApprovedVenues(selectedMembership.organization.slug)} className={buttonStyles({ variant: "outline" })}>Review approved venues</Link><p className="basis-full text-sm text-slate-600">Manage events and sessions here; open a session to review bookings, issued digital tickets, and financial reporting. Checkout in this demo is simulated — no money moves.</p></div>
      </Container>
    </section>
  );
}
